...
export function useClientAuth() {
  ...
  async function hydrate(u) {
    try {
      ...
      if (p && !error) { setUser(u); setProfile(p) }
      else {
        const name = u.user_metadata?.full_name || u.email?.split('@')[0] || 'Driver'
        const { data: newP } = await supabase.from('profiles')
          .upsert({ id:u.id, full_name:name, phone:u.user_metadata?.phone||null,
            role:'driver', is_active:true, clearance_level:1,
            avatar_initials:name[0].toUpperCase() }, { onConflict:'id' })
          .select().single()
    ...
    try {
      const { data } = await supabase.from('vehicles').select('*').eq('owner_id', u.id)
      setVehicles(data || [])
    } catch { setVehicles([]) }
  }
  ...
  function exitDemo() { setIsDemo(false); setUser(null); setProfile(null); setSession(null); setVehicles([]) }
  ...
  async function signUp({ email, password, full_name, phone }) {
    ...
    const { data, error } = await supabase.auth.signUp({ email, password, options: { data: { full_name: full_name.trim(), phone: phone||null } } })
    if (error) throw error
    return data
  }
  ...
  async function refreshVehicles() {
    ...
    try { const { data } = await supabase.from('vehicles').select('*').eq('owner_id', user.id); setVehicles(data || []) } catch {}
  }
 
  return { user, session, profile, vehicles, loading, isDemo, isAuthenticated: !!user, signIn, signUp, signOut, resetPassword, refreshVehicles, enterDemo, exitDemo }
}
