// Registration categories by Kenya plate format
export const REGISTRATION_CATEGORIES = [
  {
    id:      'private',
    label:   'Private',
    description: 'KAA 000A — standard private vehicle',
  ...
  },
  {
    id:      'government',
    label:   'Government',
    description: 'GKA 000A / GKB 000A — national government',
    pattern: /^GK[A-Z]\s?\d{3}[A-Z]$/,
    example: 'GKA 001B',
    prefix:  'GK',
  },
  {
    id:      'diplomat',
    label:   'Diplomat / CD',
    description: '29 CD 123H — diplomatic corps',
    pattern: /^\d{1,3}\s?CD\s?\d{1,4}[A-Z]?$/,
    example: '29 CD 123H',
    prefix:  'CD',
  },
  {
    id:      'police',
    label:   'Police / Administration Police',
    description: 'GK A000A — national police service',
    pattern: /^GK\s?[A-Z]\d{3}[A-Z]$/,
    example: 'GK A001B',
    prefix:  'GK',
  },
  {
    id:      'military',
    label:   'Kenya Defence Forces',
    description: 'KDF 0000 — military vehicles',
    pattern: /^KDF\s?\d{4}$/,
    example: 'KDF 0001',
    prefix:  'KDF',
  },
  {
    ...
    description: 'T 000 A — public service vehicles',
    pattern: /^T\s?\d{3}\s?[A-Z]$/,
    example: 'T 001 A',
    prefix:  'T',
  },
  {
    ...
    description: 'UN 0000 / IO 0000',
    pattern: /^(UN|IO|EU|AU)\s?\d{3,4}$/,
    example: 'UN 0001',
    prefix:  'UN',
  },
  {
    id:      'foreign',
    label:   'Foreign / Temporary',
    description: 'TZ / UG / ET plates',
    pattern: /^[A-Z]{2,3}\s?\d{3,5}[A-Z]?$/,
    example: 'UG 001 ABC',
    prefix:  '',
  },
  {
    ...
    description: 'KAA 000A — registered commercial',
    pattern: /^K[A-Z]{2}\s?\d{3}[A-Z]$/,
    example: 'KBZ 001A',
    prefix:  'K',
  },
...
export function validatePlate(raw, categoryId) {
  ...
  if (cat.pattern.test(plate)) {
    return { valid: true, type: cat.label, plate }
  }

  return {
    valid: false,
    error: `Invalid format for ${cat.label}. Expected: ${cat.example}`
  }
}
...
export const MAKES = [
  ...
  'Volvo','Lexus','Scania','Hino','MAN','Tata','Ashok Leyland','Other',
]
...
export const MODELS = {
  ...
  Honda:        ['Fit','Jazz','Civic','Accord','CR-V','HR-V','Freed','Stream','Stepwgn','Pilot'],
  Subaru:       ['Impreza','Legacy','Forester','Outback','XV','WRX'],
  Isuzu:        ['D-Max','MU-X','ELF','NPR','NQR','Forward','FSR','FVR'],
  'Mercedes-Benz': ['C-Class','E-Class','S-Class','GLE','GLC','Sprinter','Actros','Axor'],
  BMW:          ['3 Series','5 Series','7 Series','X3','X5','X6','1 Series'],
  Volkswagen:   ['Golf','Polo','Passat','Tiguan','Touareg','Amarok','Caddy','Crafter'],
  Ford:         ['Ranger','Everest','Explorer','Focus','Escape','Transit','Cargo'],
  Hyundai:      ['Tucson','Santa Fe','i10','i20','Elantra','Sonata','H100','County'],
  Kia:          ['Picanto','Rio','Sportage','Sorento','Carnival'],
  Suzuki:       ['Alto','Swift','Vitara','Jimny','Ertiga','Carry','Every'],
  ...
  Tata:         ['Xenon','Safari','Ace','Prima','Ultra'],
  Other:        ['Other'],
}
...
export const DEMO_PROFILE = {
  id: 'demo', full_name: 'Demo Driver', role: 'driver',
  clearance_level: 1, is_active: true, subscription_tier: 'premium',
}

export const DEMO_ADMIN_PROFILE = {
  id: 'demo-admin', full_name: 'Demo Admin', role: 'super_admin',
  clearance_level: 10, is_active: true,
}
